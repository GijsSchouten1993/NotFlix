...
public class Model {
	...
	public void SetTokenFromFile(String text) {
		ArrayList<Token> tokensFromFile = new ArrayList<Token>();
		String[] lines = text.split("/n");

		for (int i = 0; i < lines.length; i++) {
			if (lines[i] != null && lines[i].length() > 0) {
				String[] tempLines = lines[i].split(";");
				Token tok = new Token(Integer.parseInt(tempLines[0]), tempLines[1]);
				tokensFromFile.add(tok);
			}
		}

		this.tokens = tokensFromFile;

	}

	public String GetTokenTextForFile() {
		String text = "";
		for (Token tok : this.tokens) {

			text += (tok.getUserId() + ";" + tok.getToken() + "\n");
		}
	 ...
	 * 
	 * @param user
	 *            De user die toegevoegd moet worden
	 * @return het id van de toegevoegde user
	 * @throws Exception
	 *             geeft een bericht wanneer nickname van nieuwe user al bestaat
	 */
	 ...
	 * 
	 * @param _user
	 *            De user waarvan de nickname gecheckt moet worden
	 ...
	 * 
	 * @param _rating
	 *            de rating die gecheckt moet worden
	 ...
	 * 
	 * @param _rating
	 *            de rating die toegevoegd moet worden
	 * @return het id van de nieuwe rating
	 * @throws Exception
	 *             gooit exception als rating al bestaat
	 */
	 ...
	 * 
	 * @param ID
	 *            het id van de rating die je wilt hebben
	 ...
	 * 
	 * @param _userId
	 *            het id van de gebruiker waar je de ratings van wilt hebben
	 * @return
	 */
	 ...
	 * 
	 * @param movieId
	 *            het movieId waarvan je de ratings wilt hebben.
	 ...
	 *            de rating die gecheckt moet worden
	 * @throws Exception
	 *             geeft exception als rating niet klopt
	 */
	 ...
	 * 
	 * @param _rating
	 *            de rating die geupdate moet worden
	 * @return true als rating geupdate is en false zo niet
	 * @throws Exception
	 *             gooit exception als rating niet klopt
	 */
	 ...
	 * 
	 * @param ratingsId
	 *            het id van de rating die gedelete moet worden
	 ...
	 * @return returnt token als die bestaat en null zo niet
	 */
	public Token CheckIfTokenExistsInList(int _userid) {
		for (Token tok : this.tokens) {
			if (tok.getUserId() == _userid) {
				return tok;
			}
		}
		return null;
	 ...
	 * 
	 * @param _login
	 *            de login van de user
	 * @return de token die meegegeven wordt aan de user
	 * @throws Exception
	 *             geeft exception als login ongeldig is
	 */
	...
	/**
	 * Kijkt of het request het token bevat en de userID in de url overeenkomt
	 * met het token.
	 * 
	 * @param id
	 *            id van de user
	 * @param _token
	 *            de token die overeen moet komen met id van user
	 * @param mod
	 *            het model waarmee je het token kan checken
	 */
	public void CheckAuthorizationWithUserId(int id, String _token, Model mod) {
		Token tok = mod.CheckIfTokenExists(_token);
		if (tok == null)
			throw new ResponseWithException("Geen geldige token gevonden, vul het token in de custom header 'token'",
					Response.Status.UNAUTHORIZED);

		if (tok.getUserId() != id) {
			throw new ResponseWithException("U heeft onvoldoende rechten voor dit eindpunt",
	 ...
	 * 
	 * @param _token
	 *            het token die geldig moet zijn
	 * @param mod
	 *            het model waarmee je het token kan checken
	 */
